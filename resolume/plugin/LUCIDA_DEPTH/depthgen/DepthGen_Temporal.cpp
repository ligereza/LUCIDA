...
// Exact order-statistic percentile without a full sort. This keeps the same
// interpolated result as a sorted vector while reducing temporal quantile
// work from O(n log n) to seven bounded O(n)-average selections.
...
bool MeasureUnitQuantiles(
	...
	for (int i = 0; i < kTemporalQuantileCount; ++i) {
		quantiles[i] = PercentileFromUnsorted(&samples, kTemporalPercentiles[i]);
	}
