...
FloatImage ResizeBilinearAligned(const FloatImage& input, int out_width, int out_height) {
	...
	std::vector<ResampleCoordinate> x_coordinates(static_cast<size_t>(out_width));
	std::vector<ResampleCoordinate> y_coordinates(static_cast<size_t>(out_height));
	...
	for (int x = 0; x < out_width; ++x) {
		...
		x_coordinates[static_cast<size_t>(x)] = {
			x0, std::min(x0 + 1, input.width - 1), source_x - static_cast<float>(x0)};
	}
	for (int y = 0; y < out_height; ++y) {
		const float source_y = y * y_scale;
		const int y0 = static_cast<int>(std::floor(source_y));
		y_coordinates[static_cast<size_t>(y)] = {
			y0, std::min(y0 + 1, input.height - 1), source_y - static_cast<float>(y0)};
	}
	for (int y = 0; y < out_height; ++y) {
		...
		for (int x = 0; x < out_width; ++x) {
			...
			const float top = input.values[static_cast<size_t>(y_coordinate.low) * input.width + x_coordinate.low] * (1.0f - fx) +
				input.values[static_cast<size_t>(y_coordinate.low) * input.width + x_coordinate.high] * fx;
			const float bottom = input.values[static_cast<size_t>(y_coordinate.high) * input.width + x_coordinate.low] * (1.0f - fx) +
				input.values[static_cast<size_t>(y_coordinate.high) * input.width + x_coordinate.high] * fx;
