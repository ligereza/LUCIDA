...
class InvalidSourceAdapterError(SourceAdapterRegistryError):
    """Raised when an adapter declaration is incomplete or unsafe."""


class UnknownSourceAdapterError(KeyError):
    """Raised when routing requests an unregistered source_app."""


class UndeclaredEventTypeError(SourceAdapterRegistryError):
    """Raised when an adapter did not declare an event type."""
...
class SourceAdapterRegistry:
    ...
    def route(
        ...
        if event.source_app != source_app or event.event_type != event_type:
            raise SourceAdapterRegistryError("adapter output does not match declaration")
        return event
...
def _read_declaration(adapter: SourceAdapter) -> SourceAdapterDeclaration:
    ...
    return SourceAdapterDeclaration(source_app, event_types, capabilities)


def _read_identifier_set(
    adapter: SourceAdapter,
    field_name: str,
    *,
    allow_empty: bool,
